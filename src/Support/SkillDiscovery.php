...
class SkillDiscovery
{
     ...
     *
     * @param  array  $paths  The paths to scan for skills.
    ...
    public function resolve(string $nameOrPath): ?Skill
    {
        if (File::isDirectory($nameOrPath)) {
            ...
            if (File::exists($skillFile)) {
                try {
                    return SkillParser::parse(File::get($skillFile));
                } catch (Throwable $e) {
                    return null;
                }
            }
    ...
    /**
     * Scan the configured paths for skills.
     *
     * @return Collection<string, Skill>
     */
    protected function scan(): Collection
    {
        ...
        foreach ($this->paths as $path) {
            ...
            foreach ($finder as $file) {
                try {
                    $skill = SkillParser::parse($file->getContents());
